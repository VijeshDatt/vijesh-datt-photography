        ...
        <masonry :cols="{default: 4, 960: 1, 1264: 2}">
          <v-container v-for="(image, index) in images" :key="index">
            <v-hover v-slot="{ hover }">
              <v-img contain :src="image" class="my-0 text-center rounded-xl elevation-16 animate__animated animate__zoomIn animate__faster" @load="loaded(image)">
                <!-- Hover for zoom -->
                <transition name="scale-transition">
                  <div v-if="hover" class="d-flex transition-fast-in-fast-out grey darken-2 v-card--reveal" style="height: 100%; cursor: pointer;" @click="openImage(image)">
